...
<<<<<<< HEAD
  origin: [
    "https://hair-specialist.vercel.app",
    "http://localhost:5173",
    "http://localhost:5174",
  ], // Allow both production and local frontend
...
=======
    ...
    allowedHeaders: "Content-Type,Authorization",
    credentials: true
>>>>>>> bac6fc9fad9ea2682b02f182ac8ba901fa3d8ce1
};
...
<<<<<<< HEAD
    ...
    if (search) {
      query = query.or(
        `speciality.ilike.%${search}%,location.ilike.%${search}%`
      );
=======
        let query = supabase
            .from("specialist_profile")
...
>>>>>>> bac6fc9fad9ea2682b02f182ac8ba901fa3d8ce1
    ...
    if (error) {
      return res
        .status(500)
        .json({ message: "Error fetching specialists.", error });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching specialists." });
  }
});
...
app.get("/api/specialists/user/:user_id", async (req, res) => {
  ...
  const { data: specialist, error } = await supabase
    .from("specialist_profile")
    .select(
      "id, user_id, speciality, service_rates, location, rating, users(full_name)"
    )
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (specialistError || !specialist) {
        return res
          .status(404)
          .json({ message: "Specialist profile not found." });
      }
      ...
      if (!appointments || appointments.length === 0) {
        return res
          .status(404)
          .json({ message: "No appointments found for this specialist." });
      }
...
app.post("/api/appointments", async (req, res) => {
  ...
  try {
    // Step 1: Fetch the duration of the selected service
    ...
    // Step 2: Calculate start and end time
    const appointmentStart = new Date(`${date}T${time}`);
    const appointmentEnd = new Date(
      appointmentStart.getTime() + timing.hours * 60 * 60 * 1000
    ); // Add hours
    ...
    if (overlapError) {
      return res
        .status(500)
        .json({ error: "Error checking overlapping appointments." });
    }
    ...
    // Step 4: Insert new appointment into both tables
    const { data: appointmentEntry, error: appointmentInsertError } =
      await supabase
        .from("appointments")
        .insert([
          { customer_id, specialist_id, service_id, date, time, status },
        ])
        .select()
        .single();
    ...
    if (periodInsertError) {
      return res
        .status(500)
        .json({ error: "Failed to create appointment period." });
    }

    return res.status(201).json({ appointment: appointmentEntry });
  } catch (err) {
...
app.post("/api/reviews", authenticateToken, async (req, res) => {
  ...
  if (!rating || rating < 1.0 || rating > 5.0) {
    return res
      .status(400)
      .json({ error: "Rating must be between 1.0 and 5.0" });
  }
  ...
  async (req, res) => {
    ...
    try {
      ...
      const response = appointments.map((app) => ({
        ...
        specialist_profile: {
          speciality:
            specialistsMap[app.specialist_id]?.speciality || "Unknown",
          full_name:
            usersMap[specialistsMap[app.specialist_id]?.user_id] || "Unknown",
        },
      }));
  ...
  try {
    ...
    if (specialistProfile) {
      ...
      if (customer) {
        ...
        console.log(
          "⚠️ User is neither a specialist nor a customer. Returning empty list."
        );
    ...
    const { data: chats, error: chatsError } = await supabase
      ...
      .or(
        `client_id.eq.${customerId || -1},specialist_id.eq.${
          specialistId || -1
        }`
      )
      .order("created_at", { ascending: false });
    ...
    const chatsWithDetails = await Promise.all(
      chats.map(async (chat, index) => {
        ...
        const { data: specialistProfile, error: specialistProfileError } =
          await supabase
=======
    const userId = parseInt(req.params.userId, 10);
...
<<<<<<< HEAD
        if (specialistProfile && !specialistProfileError) {
          const { data: specialist, error: specialistNameError } =
            await supabase
              .from("users")
              .select("full_name")
...
app.post("/api/chats", async (req, res) => {
  ...
  if (!client_id || !specialist_id) {
    return res
      .status(400)
      .json({ error: "Both client_id and specialist_id are required." });
  }
...
app.post("/api/chats/create", async (req, res) => {
  ...
  if (!client_id || !specialist_id) {
    return res
      .status(400)
      .json({ error: "Client ID and Specialist ID are required." });
  }

  try {
    console.log(`📥 Received request to create or fetch chat`);
    console.log(
      `🔍 Resolving actual customer ID from user ID (client_id): ${client_id}`
    );
    ...
    if (customerError || !customer) {
      ...
      return res
        .status(400)
        .json({ error: "Customer not found for given user ID." });
    }
